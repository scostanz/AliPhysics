...
void ratio_eff_2016(){
  ...
  TCanvas *cratio16p = new TCanvas("effratio16_pmpt","effratio16_pmpt",800,600);
  TCanvas *cratio16f = new TCanvas("effratio16_fddw","effratio16_fddw",800,600);
  TH1D *hrpmpt[nper], *hrfddw[nper];
  TH1D *hr16pmpt[nper], *hr16fddw[nper];
  ...
  for (int j=1;j<3;j++) {
    hr16pmpt[j] = (TH1D*) heff_pmpt[j]->Clone();
    hr16pmpt[j]->Divide(heff_pmpt[j], heff_pmpt[0],1,1,"B");
    cratio16p->cd();
    hr16pmpt[j]->GetYaxis()->SetTitle("Efficiency ratio");
    hr16pmpt[j]->GetYaxis()->SetRangeUser(0.8,1.2);
    hr16pmpt[j]->SetTitle(Form("LHC%s%s/LHC2016",year[j].Data(),subdir[j].Data()));
    hr16pmpt[j]->Draw("same");
    hr16fddw[j] = (TH1D*) heff_fddw[j]->Clone();
    hr16fddw[j]->Divide(heff_fddw[j], heff_fddw[0],1,1,"B");
    cratio16f->cd();
    hr16fddw[j]->GetYaxis()->SetTitle("Efficiency ratio");
    hr16fddw[j]->GetYaxis()->SetRangeUser(0.8,1.2);
    hr16fddw[j]->SetTitle(Form("LHC%s%s/LHC2016",year[j].Data(),subdir[j].Data()));
    hr16fddw[j]->Draw("same");
  }
  cratio16p->BuildLegend();
  cratio16f->BuildLegend();
  cratio16p->cd(); ll->Draw("same");
  cratio16f->cd(); ll->Draw("same");
