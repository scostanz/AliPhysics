...
TString suffix[19] = {"1_rl1","1_rl2","2","3_rl1","3_rl2","4","5","6_rl1","6_rl2","7","8","9_rl1","9_rl2","10_rl1","10_rl2","11","12_rl1","12_rl2","13"};

void Merge_data_2018(){
  
  TString dirtomerge="";

  TFile * fi=new TFile(Form("%s/AnalysisResults_child%s.root",inputdir.Data(),suffix[0].Data()));
  cout << "Opening file: " << Form("%s/AnalysisResults_child%s.root",inputdir.Data(),suffix[0].Data()) << endl;
  ...
  for (int i=0; i<19;i++) {
    cout << "Adding file " << Form("%s/AnalysisResults_child%s.root",inputdir.Data(),suffix[i].Data()) << endl;
    mga.AddFile(Form("%s/AnalysisResults_child%s.root",inputdir.Data(),suffix[i].Data()));
  }
