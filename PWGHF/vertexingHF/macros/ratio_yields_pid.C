...
void ratio_yields_pid() {
  ...
  TString inputdir = "~/alice/D0_pp13TeV/results/figures/LHC2016/templ/";
...
//   TString legend[nMax] = {"2#sigma TPC 3#sigma TOF, p_{T} 400 MeV/c","2#sigma TPC 2#sigma TOF, p_{T} 400 MeV/c","3#sigma TPC 2#sigma TOF, p_{T} 400 MeV/c","central cuts"};
  TString dataset[nMax] = {"3SigPID_Pt300","3SigPID_Pt300","3SigPID_Pt400","NoPID_Pt400","3SigPID_Pt400"};
  ...
  for (Int_t i=0; i<nMax; i++) {
    if (i==0 || i==3 || i==4)
      fileName[i].Form("%s%s/%s%s_YFid_PileUpMV.root",inputdir.Data(), dataset[i].Data(),fname.Data(), dataset[i].Data());
    else 
      fileName[i].Form("%s%s_SPDoff/%s%s_YFid_PileUpMV_SPDoff.root",inputdir.Data(), dataset[i].Data(),fname.Data(), dataset[i].Data());
  ...
  TFile *fout = new TFile(Form("%s/Compare2_RawYields_PID.root",inputdir.Data()), "RECREATE");
  c->Write();
  cratio->Write();
  fout->Write();
  fout->Close();
